...
def unstabilized_strip(seq, idxs, out_name, context_factor=1.5, patch_size=224):
    ...
    for i in idxs:
        f = frames[i]
        image = np.array(Image.open(RAW / seq / "images" / (f["frame_id"] + ".jpg")).convert("RGB"))
        ...
        tiles.append(patch)
    gap = 4
    strip = Image.new("RGB", (patch_size*len(tiles)+gap*(len(tiles)-1), patch_size), "white")
    x = 0
    for t in tiles:
        strip.paste(t, (x, 0)); x += patch_size + gap
    strip.save(OUT / out_name, quality=92)
    print("wrote", OUT / out_name)

unstabilized_strip("pyronear-sdis-07_brison_290_2024-02-03T11-13-08", [0, 3, 6, 9, 12, 15, 19], "brison_patches_unstabilized.jpg")
