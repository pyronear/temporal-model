...
pkg = load_model_package(ROOT / "api/models/model.zip", extract_dir=ROOT / ".cache/temporal_model_core")
mi = pkg.model_input
mean = torch.tensor(mi["normalization"]["mean"]).view(3, 1, 1)
std = torch.tensor(mi["normalization"]["std"]).view(3, 1, 1)

meta = json.load(open(MI / "meta.json"))
T = meta["num_frames"]
...
ax.set_title(f"one tube as the transformer head receives it — 20 embeddings × 384 dims (tube logit: {logit:+.2f})")
