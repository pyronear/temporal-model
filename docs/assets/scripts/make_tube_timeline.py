...
candidates = build_tubes(
    fd, iou_threshold=tc["iou_threshold"], max_misses=tc["max_misses"]
)
f1 = filter_and_interpolate_tubes(
    candidates,
    min_tube_length=tc["infer_min_tube_length"],
    min_detected_entries=tc["min_detected_entries"],
    interpolate_gaps=False,
)
kept = filter_and_interpolate_tubes(
    merge_colocated_tubes(
        f1,
        merge_iomin=tc["merge_iomin"],
...
def obs_keys(t):
    return {
        (e.frame_idx, round(e.detection.cx, 9), round(e.detection.cy, 9))
        for e in t.entries
...
for t in kept:
    row_order += sorted(
        groups.get(t.tube_id, []), key=lambda i: candidates[i].start_frame
    )
...
for t in kept:
    ...
    if len(idxs) > 1:
        ...
        ax1.add_patch(
            FancyBboxPatch(
                (x0 - 0.35, min(rows) - 0.38),
                x_end - x0 + 0.7,
                max(rows) - min(rows) + 0.76,
        ...
        for a, b in zip(frag, frag[1:], strict=False):
            ax1.plot(
                [candidates[a].end_frame, candidates[b].start_frame],
                [row_of[a], row_of[b]],
                ls=":",
                color=color,
                lw=1.6,
...
ax1.invert_yaxis()
ax1.set_title(
    f"raw candidates from build_tubes ({len(candidates)}), "
...
ax2.set_title(
    f"kept tubes after filter + merge + interpolate ({len(kept)}) — "
    f"orange diamonds = interpolated gap entries",
    fontsize=10,
)
fig.tight_layout()
