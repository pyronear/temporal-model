...
for e in window:
    ...
    cx, cy, bw, bh = expand_bbox(
        e.detection.cx, e.detection.cy, e.detection.w, e.detection.h, 1.5
    )
    patch = Image.fromarray(
        crop_and_resize(img, norm_bbox_to_pixel_square(cx, cy, bw, bh, w, h), SIZE)
    )
    ...
    d.rectangle(
        [BORDER, BORDER, BORDER + (118 if e.is_gap else 64), BORDER + 24],
        fill=(0, 0, 0),
    )
...
W = tiles[0].width
strip = Image.new(
    "RGB", (W * len(tiles) + gap * (len(tiles) - 1), tiles[0].height), "white"
)
x = 0
for t_ in tiles:
    strip.paste(t_, (x, 0))
