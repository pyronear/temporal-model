...
for m in sorted(MI.glob("*/meta.json")):
    ...
    if not (9 <= hour <= 16):
        continue
    areas = [
        f["orig_bbox"][2] * f["orig_bbox"][3] for f in d["frames"] if f.get("orig_bbox")
    ]
...
for n, sid in enumerate(cands):
    ...
    if side > 0.35:
        continue
    mid = [e for e in gappy.entries if not e.is_gap and e.detection][
        len(gappy.entries) // 4
    ]
    img = np.array(Image.open(paths[mid.frame_idx]).convert("RGB"))
    h, w, _ = img.shape
    cx, cy, bw, bh = expand_bbox(
        mid.detection.cx, mid.detection.cy, mid.detection.w, mid.detection.h, cf
    )
    ...
    hits += 1
    print(
        f"bright={bright:5.1f} side={side:.2f} raw={len(raw):2d} f1={len(f1):2d} "
