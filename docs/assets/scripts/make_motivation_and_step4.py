...
def frame_strip(seq, idxs, out_name, width=480):
    meta = json.load(open(MI / seq / "meta.json"))
    ...
    for i in idxs:
        ...
        if f.get("orig_bbox"):
            ...
            d.rectangle([(cx-w/2)*W, (cy-h/2)*H, (cx+w/2)*W, (cy+h/2)*H], outline=RED, width=6)
...
def annotated_frame(seq, idx, out_name):
    ...
    if f.get("orig_bbox"):
        cx, cy, w, h = f["orig_bbox"]
        d.rectangle([(cx-w/2)*W, (cy-h/2)*H, (cx+w/2)*W, (cy+h/2)*H], outline=RED, width=5)
    font = load_font(28)
    d.text((12, H-80), "red: YOLO detection", fill=RED, font=font, stroke_width=2, stroke_fill="black")
    d.text((12, H-44), "orange: stabilized crop window (union x context)", fill=ORANGE, font=font, stroke_width=2, stroke_fill="black")
