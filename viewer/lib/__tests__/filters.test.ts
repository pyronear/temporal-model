...
describe("applyFilters", () => {
  it("default keeps everything", () => {
    expect(applyFilters(rows, defaultFilters()).map((x) => x.key)).toEqual([
      "a",
      "b",
      "c",
      "d",
  ...
  it("keeps only the selected correctness classes", () => {
    const f = {
      ...defaultFilters(),
      outcomes: ["kept-fp", "discarded-smoke"] as const,
    };
    expect(
      applyFilters(rows, { ...f, outcomes: [...f.outcomes] }).map((x) => x.key),
    ).toEqual(["b", "c"]);
  });
  it("filters by ground truth, verdict, camera", () => {
    expect(
      applyFilters(rows, { ...defaultFilters(), label: "smoke" }).map(
        (x) => x.key,
      ),
    ).toEqual(["a", "c"]);
    expect(
      applyFilters(rows, { ...defaultFilters(), verdict: "discard" }).map(
        (x) => x.key,
      ),
    ).toEqual(["c", "d"]);
    expect(
      applyFilters(rows, { ...defaultFilters(), camera: "cam-2" }).map(
        (x) => x.key,
      ),
    ).toEqual(["b"]);
  });
