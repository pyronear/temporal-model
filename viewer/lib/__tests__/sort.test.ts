...
describe("sortRows by probability", () => {
  ...
  it("desc puts highest first, nulls last", () => {
    expect(sortRows(rows, { col: "probability", dir: "desc" }).map((x) => x.key)).toEqual([
      "b", "d", "a", "c",
    ]);
  });
  it("asc puts lowest first, nulls still last", () => {
    expect(sortRows(rows, { col: "probability", dir: "asc" }).map((x) => x.key)).toEqual([
      "a", "d", "b", "c",
    ]);
  });
...
describe("nextSort", () => {
  it("new column starts desc; same column toggles", () => {
    expect(nextSort(null, "probability")).toEqual({ col: "probability", dir: "desc" });
    expect(nextSort({ col: "probability", dir: "desc" }, "probability")).toEqual({
      col: "probability", dir: "asc",
    });
