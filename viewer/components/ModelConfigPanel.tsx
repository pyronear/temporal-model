...
const HELP: Record<string, string> = {
  detector:
    "Stage-1 detector that proposes smoke boxes per frame; value is its source.",
  variant: "Packaged model variant name.",
  "train sha": "Git commit of the training run that produced this model.",
  aggregation:
    "Rule combining per-tube scores into the sequence keep/discard decision.",
  threshold:
    "Keep cutoff on a tube's raw classifier logit (max_logit aggregation).",
  "logistic threshold":
    "Keep cutoff on a tube's calibrated probability (logistic aggregation).",
  stabilize:
    "If true, each tube is cropped from one fixed window (stabilized).",
  "context factor":
    "How much the bbox is expanded before cropping the classifier patch.",
  "max frames":
    "Input sequence truncated to its first N frames; also caps frames per tube.",
...
export function ModelConfigPanel({ cfg }: { cfg: ModelConfig }) {
  ...
  const fields: Field[] = [
    ...
    {
      label: "train sha",
      value: (cfg.train_git_sha ?? "").slice(0, 8) || "—",
      mono: true,
    },
    ...
    {
      label: "pad",
      value: `${inf.pad_strategy ?? "—"} / min ${inf.pad_to_min_frames ?? "—"}`,
    },
  ];
      ...
      <dl className="grid grid-cols-2 gap-x-3 gap-y-2 border-t border-slate-100 px-3 py-2">
        {fields.map((f) => (
          <div
            key={f.label}
            title={HELP[f.label]}
            className={`cursor-help ${f.wide ? "col-span-2" : ""}`}
          >
            <dt className="text-[10px] uppercase tracking-wide text-slate-400">
              {f.label}
            </dt>
            <dd
              className={`text-sm break-words text-slate-800 ${f.mono ? "font-mono" : ""}`}
            >
              {f.value}
            </dd>
          </div>
